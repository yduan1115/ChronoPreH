...
FitCat <- function(time, class, future_time, true_class) {
  ...
  # 6. Naive Bayes
  try({
    m <- e1071::naiveBayes(class ~ time, data = df)
    pred <- predict(m, newdata = data.frame(time = future_time))
    prob <- predict(m, newdata = data.frame(time = future_time), type = "raw")
  ...
  # 7. Decision Tree
  try({
    m <- rpart::rpart(class ~ time, data = df, method = "class")
    pred <- predict(m, newdata = data.frame(time = future_time), type = "class")
    prob <- predict(m, newdata = data.frame(time = future_time), type = "prob")
  ...
  try({
    ...
    knn_fit <- caret::train(class ~ time, data = df, method = "knn",
                            tuneGrid = data.frame(k = 5), trControl = ctrl)
    pred <- predict(knn_fit, newdata = data.frame(time = future_time))
    prob <- predict(knn_fit, newdata = data.frame(time = future_time), type = "prob")
